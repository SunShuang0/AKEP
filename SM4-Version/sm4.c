...
void SM4_Encrypt(unsigned char MK[], unsigned char PlainText[], unsigned char CipherText[])
{
	...
	for (j = 0; j < 4; j++)
	{
		CipherText[4 * j + 0] = (X[35 - j] >> 24) & 0xff;
...
void SM4_Decrypt(unsigned char MK[], unsigned char CipherText[], unsigned char PlainText[])
{
	...
	for (j = 0; j < 4; j++)// 将密文字节数组存入int变量，大端模式
	{
	...
	for (i = 0; i < 32; i++)// 32次迭代运算
	{
		tmp = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[31 - i];// 轮密钥逆序使用
		// 非线性操作
		buf = (SM4_Sbox[(tmp >> 24) & 0xff]) << 24
			| (SM4_Sbox[(tmp >> 16) & 0xff]) << 16
			| (SM4_Sbox[(tmp >> 8) & 0xff]) << 8
	...
	for (j = 0; j < 4; j++)// 对最后一轮数据进行反序变换
	{
		PlainText[4 * j + 0] = (X[35 - j] >> 24) & 0xff;
...
int SM4_Rotl32(unsigned int buf, unsigned int n)
{
